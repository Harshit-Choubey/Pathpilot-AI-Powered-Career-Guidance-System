...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = resData?.detail || resData?.error || resData?.message || t('login.error_default', 'Invalid email or password. Please try again.');
    ...
    <div className="flex-1 flex items-center justify-center bg-slate-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full glass p-8 rounded-2xl">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-extrabold text-slate-900">{t('login.welcome_back', 'Welcome back')}</h2>
          <p className="mt-2 text-sm text-slate-600">
            {t('login.no_account', "Don't have an account?")}{' '}
            <Link to="/signup" className="font-medium text-indigo-600 hover:text-indigo-500">
              {t('login.signup_today', 'Sign up today')}
            </Link>
          </p>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">{t('login.email', 'Email address')}</label>
            <input
              type="email"
              required
              autoComplete="email"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">{t('login.password', 'Password')}</label>
