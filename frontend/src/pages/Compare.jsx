  ...
  if (isLoading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 min-h-[600px]">
        ...
        <h2 className="text-2xl font-bold text-slate-800">Loading Comparisons...</h2>
      </div>
    );
  }
    ...
    <div className="flex-1 bg-slate-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Career Comparison</h1>
          <p className="text-slate-600">Compare different career paths side-by-side to make an informed decision</p>
        </div>
                ...
                <div key={idx} className="bg-white border-2 border-indigo-200 rounded-2xl shadow-xl flex flex-col relative overflow-hidden">
                  ...
                  <div className="p-6 pb-2 relative z-10">
                    ...
                    <div className="flex items-center gap-1 text-indigo-600 font-bold mb-6">
                      <Target size={18} />
                      <span className="text-xl">92% Match</span>
                    </div>

                    <div className="space-y-4 text-sm font-medium">
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col">
                        <span className="text-slate-400 text-xs mb-1 flex items-center gap-1"><IndianRupee size={14}/> Salary Range</span>
                        <span className="text-slate-800 font-bold">{career.salary_range}</span>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col">
                        <span className="text-slate-400 text-xs mb-1 flex items-center gap-1"><TrendingUp size={14}/> Job Growth</span>
                        <span className="text-slate-800 font-bold">{career.job_growth_percentage}</span>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col">
                        <span className="text-slate-400 text-xs mb-1 flex items-center gap-1"><Clock size={14}/> Time to Start</span>
                        <span className="text-slate-800 font-bold">{career.time_to_start}</span>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col">
                        <span className="text-slate-400 text-xs mb-1 flex items-center gap-1"><Briefcase size={14}/> Experience</span>
                        <span className="text-slate-800 font-bold">{career.experience_level}</span>
                      </div>
                    </div>

                    <div className="mt-8">
                      <p className="text-xs font-bold text-emerald-500 tracking-wide uppercase mb-3 flex items-center gap-1"><CheckCircle size={14}/> Advantages</p>
                      ...
                      <p className="text-xs font-bold text-rose-500 tracking-wide uppercase mb-3 flex items-center gap-1"><XCircle size={14}/> Challenges</p>
